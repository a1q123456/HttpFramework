...
void Net::Sockets::Socket::Bind(std::string ip, uint32_t port)
{
	...
	{
		throw std::logic_error("cannot bind because state of this socket is not correct");
	}
...
void Net::Sockets::Socket::Listen(int backlog)
{
	...
	if (_socket == INVALID_SOCKET || client_mode)
	{
		throw std::logic_error("cannot listen because state of this socket is not correct");
	}
...
Async::Awaiter<int> Net::Sockets::Socket::ConnectAsync(std::string ip, uint32_t port)
{
	...
	{
		throw std::logic_error("cannot connect because state of this socket is not correct");
	}
...
bool Net::Sockets::Socket::IsDisposed() const
{
	return disposed;
}
